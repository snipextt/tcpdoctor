...
// HealthThresholds contains configurable thresholds for health indicators
type HealthThresholds struct {
	// RetransmissionRatePercent is the threshold for high retransmission warning
	// Calculated as (retransmissions / total segments sent) * 100
	RetransmissionRatePercent float64
	
	// HighRTTMilliseconds is the threshold for high RTT warning
	HighRTTMilliseconds uint32
}

// DefaultHealthThresholds returns the default health thresholds
func DefaultHealthThresholds() HealthThresholds {
	return HealthThresholds{
		RetransmissionRatePercent: 5.0,  // 5% retransmission rate
		HighRTTMilliseconds:       500,  // 500ms RTT
	}
}
